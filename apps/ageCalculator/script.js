...
function isValidDate() {
  ...
  year % 400 === 0 || year % 4 === 0 ? (validDays[1] = 29) : (validDays[1] = 28)
...
function calculateTime() {
  ...
  document.getElementById(
    'resultYears'
  ).innerHTML = `<span class="number">${years}</span> <span>years<span>`

  document.getElementById(
    'resultMonths'
  ).innerHTML = `<span class="number">${months}</span> <span>months<span>`

  document.getElementById(
    'resultDays'
  ).innerHTML = `<span class="number">${days}</span> <span>days<span>`
}
